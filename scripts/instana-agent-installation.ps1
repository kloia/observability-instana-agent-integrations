...
# replace host-1 backend-1 config (matches whatever host was set at install time,
# SaaS default or a self-hosted INSTANA_AGENT_ENDPOINT)
(Get-Content -Path $INSTANA_BACKEND_ONE_PATH) |
    ForEach-Object {$_ -Replace 'host=.*', $INSTANA_AGENT_HOST_ONE} |
        Set-Content -Path $INSTANA_BACKEND_ONE_PATH

# replace host-2 backend-2 config
(Get-Content -Path $INSTANA_BACKEND_TWO_PATH) |
    ForEach-Object {$_ -Replace 'host=.*', $INSTANA_AGENT_HOST_TWO} |
